...
| `MONGODB_APP_PASSWORD`    | `mongodb-catalog-app` (ns `ecommerce`)             | `password`, `username` (`catalog`) |

`mongodb-catalog-app` is mounted on products and read by the Kafka Connect products-outbox Mongo connector (Role in `ecommerce`). The SCRAM user is `catalog` on auth database `admin`, with readWrite on `catalog` plus `read` on `local`/`config` for change streams. There is no products Postgres secret after catalog cutover.
