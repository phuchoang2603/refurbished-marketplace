...
Staging syncs from `infra/argocd/staging/`. Local Colima syncs from `infra/argocd/local/`.
...
**Staging:** `values-staging.yaml` overlays (GHCR `:main`, production hostnames, RKE2 CNI paths).
...
Sync waves under `infra/argocd/staging/apps/` (and the same waves under `infra/argocd/local/apps/`): operators + Istio base (0) → observability + istiod/cni (1) → ztunnel (2) → marketplace (3) → kafka (4) → cloudflare-tunnel (5).
...
├── local/
│   ├── root.yaml
│   └── apps/          # operators, Istio, observability, marketplace, kafka, cloudflare-tunnel
└── staging/
    ├── root.yaml
    └── apps/          # same apps; staging valueFiles / image tags
```
