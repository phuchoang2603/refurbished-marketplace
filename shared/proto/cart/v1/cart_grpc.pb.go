...
func (c *cartServiceClient) SetCartItemQuantity(ctx context.Context, in *SetCartItemQuantityRequest, opts ...grpc.CallOption) (*Cart, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(Cart)
	err := c.cc.Invoke(ctx, CartService_SetCartItemQuantity_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _CartService_SetCartItemQuantity_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(SetCartItemQuantityRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(CartServiceServer).SetCartItemQuantity(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: CartService_SetCartItemQuantity_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(CartServiceServer).SetCartItemQuantity(ctx, req.(*SetCartItemQuantityRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var CartService_ServiceDesc = grpc.ServiceDesc{
	...
	Metadata: "shared/proto/cart/v1/cart.proto",
}
