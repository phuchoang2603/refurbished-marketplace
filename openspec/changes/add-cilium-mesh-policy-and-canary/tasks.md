...
- [x] 2.1 Add CiliumNetworkPolicies (observe then enforce) for documented caller pairs; deny unknown identities.
...
- [x] 3.1 Configure retry/timeout/outlier settings via Gateway API and/or CiliumEnvoyConfig for safe hops only.
- [x] 3.2 Confirm CreateOrder / hosted-payment callback are not retried unsafely; align with existing idempotency.
