...
- Document and verify the telemetry path for request metrics, service graph, and protocol visibility.
...
- Affects Kubernetes platform manifests under `infra/argocd/` and Helm values/templates under `infra/charts/refurbished-marketplace/`.
- Adds Istio as a cluster dependency managed through GitOps for staging first, with production following the same pattern when ready.
