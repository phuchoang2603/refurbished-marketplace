...
func reserveOrderItems(ctx context.Context, q *database.Queries, orderID uuid.UUID, items []ReservationItemInput) error {
	...
	for _, item := range items {
		if _, err := q.ReserveInventoryStock(ctx, database.ReserveInventoryStockParams{
			ProductID: item.ProductID,
			Quantity:  item.Quantity,
		}); err != nil {
			return err
		}
		if _, err := q.CreateInventoryReservation(ctx, database.CreateInventoryReservationParams{
			OrderID:   orderID,
			ProductID: item.ProductID,
			Quantity:  item.Quantity,
...
func createInventoryReservedOutbox(ctx context.Context, q *database.Queries, orderID, merchantID uuid.UUID, totalCents int64) error {
	payload, err := proto.Marshal(&inventoryv1.InventoryReserved{
	...
	_, err = q.CreateInventoryOutbox(ctx, database.CreateInventoryOutboxParams{
		ID:          uuid.New(),
		AggregateID: orderID,
		EventType:   messaging.EventTypeInventoryReserved,
		Payload:     payload,
	})
	return err
}
...
func createInventoryReservationFailedOutbox(ctx context.Context, q *database.Queries, orderID uuid.UUID) error {
	payload, err := proto.Marshal(&inventoryv1.InventoryReservationFailed{OrderId: orderID.String()})
	if err != nil {
		return err
	}
	...
	_, err = q.CreateInventoryOutbox(ctx, database.CreateInventoryOutboxParams{
		ID:          uuid.New(),
		AggregateID: orderID,
		EventType:   messaging.EventTypeInventoryReservationFailed,
		Payload:     payload,
	})
	return err
}
